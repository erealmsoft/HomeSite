...
exports.sendProject = function(req, res) {
    ...
    mailer.sendTemplate(name + '<' + email + '>', 'messagePlanReciced',{language:language,fullName: name});
    mailer.sendTemplate(config.support, 'newProjectplanMessage',{language:language, name: name, email:email, message: message,company:company,start:start,type:type,deadline:deadline,telephone:telephone,budget:budget});
    console.log(storagekey);

    //save the email message into mongodb
    require('./saveEmail').saveMessage({language:language, name: name, email:email, message: message,company:company,start:start,type:type,deadline:deadline,telephone:telephone,budget:budget});
